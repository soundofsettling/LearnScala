...
object Anagrams extends AnagramsInterface {
   ...
   *
   */
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] = dictionary.groupBy(f => wordOccurrences(f))
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    val occurencesList = occurrences.map( x =>
    {
      for {
        i <- 1 to x._2
      } yield (x._1, i)
    ...
    occurencesList.foldRight(List[Occurrences](Nil))((x,y) => y ++ (for(i <- x; j <- y) yield (i :: j)));
  }
...
object Dictionary {
  def loadDictionary: List[String] = {
    val wordstream = Option {
      getClass.getResourceAsStream(List("anagrams", "linuxwords.txt").mkString("/", "/", ""))
    } getOrElse {
