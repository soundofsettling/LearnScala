package typesandpatternmatching
...
trait Huffman extends HuffmanInterface {
   ...
   *   }
   */
  def times(chars: List[Char]): List[(Char, Int)] = (chars map (char => (char, chars count {_ == char}))).distinct
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case left :: right :: tail =>
      (makeCodeTree(left, right) :: tail).sortWith((tree1, tree2) => weight(tree1) < weight(tree2))
    case _ => trees
  }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = text match {
    case List() => List()
    case x1 :: x2 => helper(tree, List())(x1) ::: encode(tree)(x2)
  }

  def helper(tree: CodeTree, bits: List[Bit])(text: Char): List[Bit] = tree match {
    case Fork(l, r, _, _) => {
      if(chars(l).contains(text)){
        helper(l, bits.appended(0))(text)
      }else {
        helper(l, bits.appended(1))(text)
      }
    }
